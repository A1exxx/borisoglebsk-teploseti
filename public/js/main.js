...
(function () {
  ...
  // Показываем только известные сообщения сервера: иначе по ссылке с чужим
  // текстом в адресе его вывел бы официальный сайт.
  var SERVER_ERRORS = [
    'Проверьте заполнение формы',
    'Слишком много обращений за короткое время. Повторите отправку через несколько минут.',
    'Не удалось обработать запрос. Попробуйте ещё раз или сообщите в абонентский отдел.',
    'Файл больше 10 МБ. Уменьшите размер и попробуйте снова.',
    'Можно приложить не более 10 файлов.',
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!error) return;
    var text = SERVER_ERRORS.indexOf(error) !== -1 ? error : 'Проверьте заполнение формы и отправьте ещё раз.';
    var form = document.querySelector('form[data-ajax]');
    if (form) setNote(form, 'err', 'Заявка не отправлена', text);
  });
