...
| Что | Где |
|---|---|
| Объёмный город: кварталы, дома, улицы, река, железная дорога; камера из вида сверху наклоняется в объём и облетает город; то в одном, то в другом квартале загорается тепло | `public/js/hero-twin.js` (WebGL без библиотек) |
| Город строит сама программа (`makeCity`, постоянное зерно — всегда один и тот же), это не карта и не чужие данные: ничего не скачивается и подписи об источнике не нужно | `public/js/hero-twin.js` |
| Тёмный первый экран без надписей поверх; мышь на сцену не влияет; разделы главной пронумерованы | `src/pages/index.html`, `public/css/style.css` раздел 33 |
...
устройствах (остаётся тёмный фон с сеткой из CSS), не готовится в версии для
слабовидящих; при «уменьшить движение» и без видеокарты рисуется один
неподвижный кадр. На телефоне город меньше (2,6 тыс. домов против 6,6 тыс.)
и 30 кадров в секунду. Тёплые кварталы появляются в той части экрана, где
город хорошо виден, а не под текстом.

Прежняя версия с настоящим Борисоглебском по данным OpenStreetMap сохранена
меткой `approved-2026-09-19-110731-demo-variant-4-city-map`; для неё по
лицензии обязательна подпись «© участники OpenStreetMap» со ссылкой на
openstreetmap.org/copyright. Её данные собирались `tools/city-query.overpassql`
и `tools/city-data.js` в `public/data`.
