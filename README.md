...
## Первый экран: объёмный город

С 19.09.2026 на рабочем сайте оформление «Цифровой двойник» (вариант 4 из демо).

| Что | Где |
|---|---|
| Объёмный город на WebGL: камера из вида сверху наклоняется в объём и облетает город, то в одном, то в другом квартале загорается тепло | `public/js/hero-twin.js` |
| Город свой — его строит программа (`makeCity`, постоянное зерно), это не карта и не чужие данные: ничего не скачивается, подписи об источнике не нужно | `public/js/hero-twin.js` |
| Панель «На сегодня»: время по Москве, окно приёма показаний с полоской месяца, объявленные работы | `src/pages/index.html`, `public/js/main.js` раздел 9b |
| Шрифты Geist и Geist Mono (OFL 1.1) лежат на сайте, внешних запросов нет | `public/fonts` |
| Стили оформления | `public/css/style.css` разделы 30–33 |

Сцена не запускается без WebGL, при экономии трафика и на совсем слабых
устройствах (остаётся тёмный фон с сеткой из CSS) и не готовится в версии для
слабовидящих. При «уменьшить движение» и без видеокарты рисуется один
неподвижный кадр. На телефоне город меньше и 30 кадров в секунду. Кнопка
«Остановить анимацию» в панели запоминает выбор. Мышь на сцену не влияет.

Демо других вариантов оформления — ветки `demo/oformlenie*`, сборка
`node tools/build-demo.js [папка] --variant N`, публикация на GitHub Pages
из ветки `gh-pages`.

## Улучшения 19.09.2026: скорость, поиск, безопасность

| Что | Где |
|---|---|
| Сжатие (gzip) стилей и скриптов, HTTP/2, `www` → основной адрес (301), лимит запросов к панели 30 в минуту | nginx, копия конфига — `ops/nginx-teploseti.conf` |
| Полоса объявления вставляется сервером сразу в страницу — страница не «прыгает» после загрузки, объявление видят поисковики | `server/server.js`, раздел «Страницы» |
| «Показывать до» у объявления: после даты полоса сама исчезает | панель → «Объявление в шапке сайта» |
| Работа с прошедшим плановым сроком на сайте считается завершённой; в панели остаётся как есть и подсвечена — продлили работы, поменяйте дату | `publicContent()` в `server/server.js`, `server/admin/admin.js` |
| «Заявка отправлена»: номер ставит сервер, текст берётся по виду заявки, а не из адреса (раньше по ссылке с чужим текстом его показывал бы сайт) | `server/server.js`, `src/pages/spasibo.html` |
| Сообщение об ошибке формы из адреса показывается, только если это известный текст сервера | `public/js/main.js`, раздел 8 |
| Панель управления: скрипт отдельным файлом — встроенный блокировала защита сайта (CSP), и с 10.09.2026 панель не загружала и не сохраняла данные | `server/admin/admin.js` |
| После 10 неверных паролей с одного адреса вход в панель закрыт на 15 минут | `requireAdmin()` в `server/server.js` |
| Полоса объявления не меняет высоту, пока грузится шрифт: ширина её текста задана в em, а не в ch. ch считается по цифре «0» из латинской части Geist; пока та не пришла, ch меньше, строка переносилась на лишнюю строку и сдвигала весь экран. Обе части Geist (кириллица и латиница) предзагружаются | `public/css/style.css` (полоса), `src/layout.html` |
| Город на первом экране строится кусками по 8 мс, шейдеры собираются в фоне — слабый телефон не «задумывается» | `public/js/hero-twin.js` |
| express 4.22.3 (закрыты уязвимости qs), `npm audit` — 0 | `server/package.json` |

Проверка сервера после правок — автотесты в сессии разработки: объявление и
дата «показывать до», автозавершение работ, номер заявки и отброс чужого
текста, блокировка подбора пароля.
