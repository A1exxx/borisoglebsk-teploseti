...
**Вариант 3 — «Изотермы»** (ветка `demo/oformlenie-v3`, демо в `/v3/`).

| Что | Где |
|---|---|
| Линии одинаковой температуры: поле течёт вверх, курсор греет, щелчок пускает волну | `public/js/hero-iso.js` (один фрагментный шейдер WebGL, без библиотек) |
| Светлый первый экран, панель «На сегодня» из матового стекла, полоска месяца с окном приёма показаний | `src/pages/index.html`, `public/css/style.css` раздел 33, `main.js` раздел 9b |
| Шрифты Onest и Geist Mono (OFL 1.1) — лежат на сайте, внешних запросов нет | `public/fonts` |

Без WebGL, при экономии трафика и на совсем слабых устройствах остаётся
спокойный фон из CSS; при «уменьшить движение» и без видеокарты
(WebGL рисует процессор) шейдер рисует один неподвижный кадр. На телефоне 30 кадров в секунду; если устройство не
успевает, разрешение холста снижается само. Размытие под панелью — только
на компьютере: на телефоне оно дорогое.

```
node tools/build-demo.js ../teploseti-gh-pages --variant 3
```
